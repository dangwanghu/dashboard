<template>
    <div class="container passwd">
        <nav class="bar bar-nav" style="background: #f6383a;opacity: 0.9;">
            ...
            <h1 class="title" style="color: #fff;">提现密码设置</h1>
        ...
        <div class="content">
            <div class="list-block">
                <ul>
                    <li>
                        <div class="item-content" v-show="password != null">
...
export default {
  ...
  methods: {
    save () {
        // 校验数据
      if (this.password && $('#originPwd').val() === '') {
        $.toast('请填写原密码')
        return false
      }
      if ($('#newPwd').val() === '') {
      ...
      if (this.password && $('#originPwd').val() !== this.password) {
        $.toast('原密码错误')
        return false
      }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.statusCode !== 200) {
                    ...
                    self.password = data.body.withdrawPassword
                }
                $.hidePreloader()
            },
            error: function (xhr) {
